...
public class SysUserController extends BaseController {
    ...
    public String add(ModelMap mmap) {
        ...
        sysRole.setIsEnable(Constants.YES);
    ...
    public String center(ModelMap mmap) {
        ...
        mmap.addAttribute("userInfo", currentUser.getSysUser());
    ...
    public ResponseInfo updateInfo(@RequestBody SysUser sysUser) {
        ActiverUser userInfo = UserUtil.getCurrentUser();
        sysUser.setId(userInfo.getSysUser().getId());
        sysUserService.updateSysUserByCondtion(sysUser);
        //重新刷新
        SysUser result = sysUserService.selectSysUserById(userInfo.getSysUser().getId());
        //刷新缓存中的用户信息
        UserUtil.refreshUser(result);
    ...
    public ResponseInfo updateAvatar(SysUser sysUser) {
        ActiverUser currentUser = UserUtil.getCurrentUser();
        sysUser.setId(currentUser.getSysUser().getId());
        sysUserService.updateSysUserByCondtion(sysUser);
        //刷新缓存中的用户信息
        //currentUser.setAvatar(sysUser.getAvatar());
        SysUser currentUserSysUser = currentUser.getSysUser();
        currentUserSysUser.setAvatar(sysUser.getAvatar());
        UserUtil.refreshUser(currentUserSysUser);
        return ResponseInfo.success("更新成功！");
    }
    ...
    public ResponseInfo updatePassword(@Validated EditPasswordReqDto editPasswordReqDto) {
        //先验证旧密码是否 正确
        ActiverUser currentUser = UserUtil.getCurrentUser();
        if (!StrUtil.equals(currentUser.getSysUser().getPassword(), SecureUtil.md5(editPasswordReqDto.getOldPassword()))) {

...
public class WebSocketServer {
    ...
    public void onMessage(String message, Session session) {
        try {
            ...
            //来源用户
            Map srcUser = (Map) hashMap.get("mine");
            //目标用户
     ...
     * 私聊
     */
    public void privateChat(Session session, Map tarUser, HashMap hashMap) throws IOException {
        //获取目标用户的session
        ...
        if (tarUserSession == null) {
            ...
            tarUserSession.getBasicRemote().sendText(JSONUtil.toJsonStr(hashMap));
        }
    }
     ...
     * 群聊
     */
    public void groupChat(Session session, HashMap hashMap) throws IOException {
        for (Entry<String, Session> entry : sessionMap.entrySet()) {
            //自己就不用再发送消息了
            if (entry.getValue() != session) {
                hashMap.put("type", "2");
                //entry.getValue().getBasicRemote().sendText(new ObjectMapper().writeValueAsString(hashMap));
                entry.getValue().getBasicRemote().sendText(JSONUtil.toJsonStr(hashMap));
            }
        }
    }
     ...
     * 通知除了自己之外的所有人的方法
     */
    public void sendOnlineCount(Session session, String message) {
        for (Entry<String, Session> entry : sessionMap.entrySet()) {
            try {
                if (entry.getValue() != session) {
    ...
    public List<SysUser> getOnlineList() {
        ...
        List<SysUser> users = userService.selectSysUserList(sysUser);
        return users;
    }

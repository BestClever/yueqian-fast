...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var elemChatMain = ['<li {{ d.mine ? "class=layim-chat-mine chat-list" : "class=layim-chat-he chat-list" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>'
    ,'<div class="layim-chat-user"><img {{# if(d.avatar){ }} src="{{ d.avatar }}" {{# }else{ }} src="/component/layui/css/modules/layim/skin/logo.jpg" {{# } }}><cite>'
    ,'{{# if(d.mine){ }}'
  ...
  ,'</li>'].join('');
  // modify by flowerStone 2022年03月15日 0015 22:09:08 修改默认图像
  var elemChatList = '<li class="layim-{{ d.data.type }}{{ d.data.id }} layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img {{# if(d.avatar){ }} src="{{ d.avatar }}" {{# }else{ }} src="/component/layui/css/modules/layim/skin/logo.jpg" {{# } }}><span>{{ d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>';
  ...
  var layimChat, layimMin, chatIndex, To = {}, popchat = function(data){
    ...
    if(chat[0]){
      ...
      if(list.find('li').length === 1 && !listThat[0]){
        ...
        // modify by flowerStone 2022年03月15日 0015 22:09:59 向左边缩小
        layimChat.css('left', 230)
  ...
  var sendMessage = function(){
    var data = {
      ...
      ,avatar: cache.mine ? cache.mine.avatar : (layui.cache.dir+'css/modules/layim/skin/logo.jpg') // modify by flowerStone 2022年03月15日 0015 22:17:39 修改默认头像
            ...
            post(members, function(res){
              layui.each(res.list, function(index, item){
                ...
                if(item.avatar){
                  ...
                  avatar = "/component/layui/css/modules/layim/skin/logo.jpg"
                }
                // modify by flowerStone 2022年03月15日 0015 22:05:34 修改默认图像
                li += '<li data-uid="'+ item.id +'"><a href="javascript:;"><img src="'+avatar +'"><cite>'+ item.username +'</cite></a></li>';

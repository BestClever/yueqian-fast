layui.define(['table', 'jquery', 'element'], function(exports) {
	...
	yueqianMenu.prototype.click = function(clickEvent) {
		...
		$("body").on("click", "#" + _this.option.elem + " .site-demo-active", function() {
			...
			if ($("#" + _this.option.elem).is(".yueqian-nav-mini")) {
	...
	yueqianMenu.prototype.collaspe = function(time) {
		...
		if ($("#" + this.option.elem).is(".yueqian-nav-mini")) {
			...
			$("#" + this.option.elem).removeClass("yueqian-nav-mini");
			...
			$("#" + this.option.elem).addClass("yueqian-nav-mini");
			$("#" + this.option.elem).animate({
				width: "60px"
			}, 400);
	...
	function createMenu(option) {
		var menuHtml = '<div style="height:100%!important;" class="yueqian-side-scroll layui-side-scroll ' + option.theme + '"><ul lay-filter="' + option.elem +
			'" class="layui-nav arrow   yueqian-menu layui-nav-tree yueqian-nav-tree">'
	...
	function createMenuAndControl(option) {
		var control = '<div style="width: '+ option.controlWidth +'px;white-space: nowrap;overflow-x: scroll;overflow: hidden;" class="control"><ul class="layui-nav yueqian-nav-control pc layui-hide-xs" style="width: fit-content;">';
		var controlPe = '<ul class="layui-nav yueqian-nav-control layui-hide-sm">';
		// 声 明 头 部
		...
		$.each(option.data, function(i, item) {
			...
			if (i === option.defaultMenu) {
				controlItem = '<li yueqian-href="' + item.href + '" yueqian-title="' + item.title + '" yueqian-id="' + item.id +
					'" class="layui-this layui-nav-item"><a href="#">' + item.title + '</a></li>';
				menuItem = '<ul  yueqian-id="' + item.id + '" lay-filter="' + option.elem +
					'" class="layui-nav arrow layui-nav-tree yueqian-nav-tree">';
				// 兼容移动端
				controlPe += '<li class="layui-nav-item"><a class="pe-title" href="javascript:;" >' + item.title + '</a>';
				controlItemPe += '<dd  yueqian-href="' + item.href + '" yueqian-title="' + item.title + '" yueqian-id="' + item.id +
					'"><a href="javascript:void(0);">' + item.title + '</a></dd>';
			} else {
				controlItem = '<li  yueqian-href="' + item.href + '" yueqian-title="' + item.title + '" yueqian-id="' + item.id +
					'" class="layui-nav-item"><a href="#">' + item.title + '</a></li>';
				menuItem = '<ul style="display:none" yueqian-id="' + item.id + '" lay-filter="' + option.elem +
					'" class="layui-nav arrow layui-nav-tree yueqian-nav-tree">';
		...
		$("#" + option.control + " .yueqian-nav-control").on("click", "[yueqian-id]", function() {
			$("#" + option.elem).find(".yueqian-nav-tree").css({
				display: 'none'
			});
			$("#" + option.elem).find(".yueqian-nav-tree[yueqian-id='" + $(this).attr("yueqian-id") + "']").css({
				display: 'block'
			});
			$("#" + option.control).find(".pe-title").html($(this).attr("yueqian-title"));
			$("#" + option.control).find("")
			option.change($(this).attr("yueqian-id"), $(this).attr("yueqian-title"), $(this).attr("yueqian-href"))
		})
	}
	...
	function downShow(option) {
		$("body #" + option.elem).on("click", "a[menu-type='0']", function() {
			if (!$("#" + option.elem).is(".yueqian-nav-mini")) {
	...
	function isHoverMenu(b, option) {
		if (b) {
			$("#" + option.elem + ".yueqian-nav-mini .layui-nav-item,#" + option.elem + ".yueqian-nav-mini dd").hover(function(e) {
				e.stopPropagation();
				var _this = $(this);

layui.define(['message', 'table', 'jquery', 'element', 'yaml', 'form', 'tab', 'menu', 'frame', 'theme', 'convert'],
	function(exports) {
		...
		var yueqianAdmin = new function() {
			...
			this.render = function(initConfig) {
				if (initConfig !== undefined) {
					...
					applyConfig(yueqianAdmin.readConfig());
				}
			}
		...
		body.on("click", ".collaspe,.yueqian-cover", function() {
			collaspe();
		});
		...
		function bottomTool() {
			return "<button id='closeTheme' style='position: absolute;bottom: 20px;left: 20px;' class='yueqian-btn'>关闭</button>"
		}
		...
		function applyConfig(param) {
			config = param;
			yueqianAdmin.logoRender(param);
			yueqianAdmin.menuRender(param);
			yueqianAdmin.bodyRender(param);
			yueqianAdmin.themeRender(param);
			yueqianAdmin.keepLoad(param);
			if (param.header.message != false) {
				yueqianAdmin.messageRender(param);
			}
		}

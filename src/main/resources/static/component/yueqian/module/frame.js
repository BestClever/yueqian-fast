layui.define(['table', 'jquery', 'element'], function (exports) {
	...
	yueqianFrame.prototype.changePageByElement = function(elem,url,title,loading){
		if(loading){
			var loading = $("#"+elem).find(".yueqian-frame-loading");
			loading.css({display:'block'});
		}
	...
	function createFrameHTML(option){
		var header = "<div class='yueqian-frame-title'><div class='dot'></div><div class='title'>"+option.title+"</div></div>"
		var iframe = "<iframe class='yueqian-frame-content' style='width:100%;height:100%;'  scrolling='auto' frameborder='0' src='"+option.url+"' ></iframe>";
		var loading = '<div class="yueqian-frame-loading">'+
			'<div class="ball-loader">'+
			'<span></span><span></span><span></span><span></span>'+
			'</div>'+
			'</div></div>';
		$("#"+option.elem).html("<div class='yueqian-frame'>"+header+iframe+loading+"</div>");
	}

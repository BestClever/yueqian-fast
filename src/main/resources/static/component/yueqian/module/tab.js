layui.define(['jquery', 'element'], function(exports) {
	...
	yueqianTab.prototype.render = function(opt) {
		...
		if (option.session) {
			// 替换 opt.data 数据
			if (sessionStorage.getItem(option.elem + "-yueqian-tab-data") != null) {
				tabData = JSON.parse(sessionStorage.getItem(option.elem + "-yueqian-tab-data"));
				option.data = JSON.parse(sessionStorage.getItem(option.elem + "-yueqian-tab-data"));
				tabDataCurrent = sessionStorage.getItem(option.elem + "-yueqian-tab-data-current");
		...
		option.success(sessionStorage.getItem(option.elem + "-yueqian-tab-data-current"));
	...
	yueqianTab.prototype.click = function(callback) {
		...
		element.on('tab(' + this.option.elem + ')', function(data) {
			...
			sessionStorage.setItem(option.elem + "-yueqian-tab-data-current", id);
			callback(id);
		});
	}
	...
	yueqianTab.prototype.addTab = function(opt) {
		...
		if (opt.close) {
			...
			title += '<span class="yueqian-tab-active"></span><span class="disable-close">' + opt.title +
				'</span><i class="layui-icon layui-unselect layui-tab-close">ဆ</i>';
		}
		...
		tabData.push(opt);
		sessionStorage.setItem(this.option.elem + "-yueqian-tab-data", JSON.stringify(tabData));
		sessionStorage.setItem(this.option.elem + "-yueqian-tab-data-current", opt.id);
	...
	yueqianTab.prototype.addTabOnlyByElem = function(elem, opt, time) {
		...
		if ($(".layui-tab[lay-filter='" + elem + "'] .layui-tab-title li[lay-id]").length <= 0) {
			...
			tabData.push(opt);
			sessionStorage.setItem(elem + "-yueqian-tab-data", JSON.stringify(tabData));
		} else {
			var isData = false;
		...
		sessionStorage.setItem(elem + "-yueqian-tab-data-current", opt.id);
	...
	function tabDelete(elem, id, callback, option) {
		...
		if (!removeTab.hasClass("layui-this")) {
			...
			tabData = JSON.parse(sessionStorage.getItem(elem + "-yueqian-tab-data"));
			tabDataCurrent = sessionStorage.getItem(elem + "-yueqian-tab-data-current");
		...
		sessionStorage.setItem(elem + "-yueqian-tab-data", JSON.stringify(tabData));
		sessionStorage.setItem(elem + "-yueqian-tab-data-current", currId);
	...
	function createTab(option) {
		...
		var tab = '<div class="yueqian-tab ' + types + type + ' layui-tab" lay-filter="' + option.elem +
			'" lay-allowClose="true">';
		...
		$.each(option.data, function(i, item) {
			...
			if (option.index == index) {
				TitleItem += '<li lay-id="' + item.id + '" class="layui-this"><span class="yueqian-tab-active"></span>';
			} else {
				TitleItem += '<li lay-id="' + item.id + '" ><span class="yueqian-tab-active"></span>';
			}
